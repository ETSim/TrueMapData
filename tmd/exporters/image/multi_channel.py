...
def create_roughness_map(
    ...
    **kwargs
    ...
    if method == "constant":
        ...
        # Normalize gradient
        gradient_min = np.min(gradient_magnitude)
        gradient_max = np.max(gradient_magnitude)
        
        if gradient_max > gradient_min:
            normalized_gradient = (gradient_magnitude - gradient_min) / (gradient_max - gradient_min)
            return min_roughness + (max_roughness - min_roughness) * normalized_gradient
        else:
            return np.ones_like(height_map) * min_roughness
