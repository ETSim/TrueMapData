...
# Import necessary TMD modules for loading TMD files
from tmd import TMD  # Changed from TMDProcessor
...
def demonstrate_height_map():
    ...
    # Use default backend by passing None to avoid backend issues
    ...
    mesh = plotter.plot_height_map(
        ...
        min_scale=0.0,      # Changed from 0.1 to 0.0
        max_scale=100.0     # Changed from 5.0 to 100.0
    )
...
def demonstrate_point_cloud():
    ...
    # Create a more complex point cloud (torus)
    n_points = 10000
...
def demonstrate_surface_comparison():
    ...
    def comparison_callback():
        ...
        _, new_scale = psim.SliderFloat("Surface 2 Height", 
                                       surface2_data['scale'], 
                                       v_min=0.0,        # Changed from 0.1 to 0.0
                                       v_max=100.0)      # Changed from 5.0 to 100.0
...
def demonstrate_interactive_features():
    ...
    mesh = plotter.plot_height_map(
        ...
        add_height_slider=True,
        min_scale=0.0,      # Changed from 0.1 to 0.0
        max_scale=100.0     # Changed from 10.0 to 100.0
    )
    ...
    def interactive_callback():
        ...
        _, height_exag = psim.SliderFloat("Height Exaggeration", 
                                         ...
                                         v_min=0.0,       # Changed from 0.1 to 0.0
                                         v_max=100.0)     # Changed from 10.0 to 100.0
...
def demonstrate_real_heightmap(file_path=None):
    ...
    try:
        # Load the TMD file using the TMD class instead of TMDProcessor
        tmd_data = TMD(file_path)
        
        # Get the height map data using the height_map() method
        ...
        # No need to set up a separate callback as it's now handled by the plotter's UI callback
...
def main():
    ...
    if args.demo == 'real' or (args.file is not None):
    ...
    elif args.demo in ['all', 'height_map']:
        print("\n" + "="*20 + " HEIGHT MAP DEMO " + "="*20)
        demonstrate_height_map()
    
        if args.demo in ['all', 'point_cloud']:
        ...
        if args.demo in ['all', 'interactive']:
...
# Create a link to the examples directory
if __name__ == "__main__":
    ...
    # Create a link to this file in the examples directory
    target_path = os.path.join(examples_dir, 'demo_polyscope.py')
