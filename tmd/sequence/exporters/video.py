    ...
    def export(self, **kwargs) -> Optional[str]:
        ...
        try:
            ...
            if show_progress:
                ...
                anim = TqdmAnimation(fig, update_frame, frames=len(frames), interval=1000 / fps, blit=True)
            else:
                anim = animation.FuncAnimation(fig, update_frame, frames=len(frames), interval=1000 / fps, blit=True)
            ...
            writer = animation.FFMpegWriter(
                fps=fps, codec=codec, metadata=dict(title="Height Map Animation"), **writer_kwargs
            )
