...
def convert_heightmap_to_displacement_map(height_map, filename="displacement_map.png", units=None):
    ...
    if units:
        metadata = {
            "Height_Range": f"{hmin:.2f} to {hmax:.2f} {units}",
            "Units": units
        }
...
def convert_heightmap_to_bump_map(height_map, filename="bump_map.png", strength=1.0, blur_radius=1.0):
    ...
    bump_map = ((height_map - h_min) / (h_max - h_min)) if h_max > h_min else np.zeros_like(height_map)
    bump_map *= strength
...
def convert_heightmap_to_multi_channel_map(height_map, filename="material_map.png", channel_type="rgbe"):
    """
    ...
    if channel_type.lower() == "rgbe":
        ...
        h_min = np.min(height_map)
        h_max = np.max(height_map)
        height_norm = ((height_map - h_min) / (h_max - h_min)) if h_max > h_min else np.zeros_like(height_map)
...
def create_terrain_type_map(height_map, terrain_type, filename="terrain_map.png"):
    ...
    normalized = ((height_map - h_min) / (h_max - h_min)) if h_max > h_min else np.zeros_like(height_map)
    ...
    if terrain_type.lower() == "mountain":
        ...
        for _ in range(int(rows / 10)):
            ...
            draw.ellipse((x - radius, y - radius, x + radius, y + radius), fill=np.random.randint(180, 230))
...
def generate_maps_from_tmd(height_map, tmd_metadata, output_dir="."):
    ...
    logger.info(f"Dimensions: {x_length}x{y_length} {units}, pixel size: {pixel_size_x:.4f}x{pixel_size_y:.4f} {units}/pixel")
    ...
    normalized_slope = ((slope_map - slope_min) / (slope_max - slope_min) * 255).astype(np.uint8) if slope_max > slope_min else np.zeros_like(slope_map, dtype=np.uint8)
    ...
    edge_map = generate_edge_map(np.array(maps["displacement"]), 
    ...
    maps["terrain_type"] = create_terrain_type_map(
        height_map, 
        terrain_type,
        filename=os.path.join(output_dir, "terrain_type.png")
    )
    ...
    map_metadata = {
        ...
        "roughness": {
            "rms": float(rms_roughness)
        },
        "slope": {
            "max_angle": float(np.arctan(slope_max) * 180/np.pi) if slope_max > 0 else 0.0
        },
