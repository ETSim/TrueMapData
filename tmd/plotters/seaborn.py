...
try:
    ...
    HAS_SEABORN = False
    raise ImportError("Seaborn is required for this module. Install with: pip install seaborn")
...
def plot_heightmap_heatmap(
    ...
    if annot and (height_map.shape[0] > 50 or height_map.shape[1] > 50):
        ...
        print(f"Downsampling large height map from {height_map.shape} to {height_map_display.shape} for display")
    else:
...
def plot_profile_comparison(
    ...
    for i, profile in enumerate(profiles):
        ...
        if fill:
            ax.fill_between(x, np.zeros_like(profile), profile, color=colors[i], alpha=0.2)
